...
- `project.yaml`、Rule、Decision、Binding、Relation 与 Evidence 使用逐类型严格 v1 Schema，拒绝未知字段、非法枚举和断链引用；ChangePlan 继续使用独立严格 Schema。
...
| 命令 | 结果 | 覆盖 |
|---|---:|---|
| `npm test` | 56 通过 | A01～A22 固定夹具证据，包括三阶段发布恢复、证据绑定、二层关系/循环、对账降级、严格对象 Schema、兼容门、重试预算，以及既有测试 |
...
- T02：业务对象和 ChangePlan 已有逐类型严格 Schema、跨类型引用校验、状态日志与幂等恢复；备份/校验等维护操作按目标路径与 manifest 保持可重复，不复用业务 Change 幂等键。
...
- T06：当前按首个 JSON/Rule 夹具生成正式对象；三类 commit 中断点与正式对象 Schema 已覆盖，尚未覆盖通用 Rule 字段变更和持久化索引重建。
