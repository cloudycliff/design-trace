...
export class OperatorServer {
  ...
  private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    const match = /^\/review\/execution\/(REV-[A-F0-9]{20})$/u.exec(url.pathname);
    if (!match?.[1]) {
    ...
    if (request.method === "GET") {
      ...
      this.respond(
        response,
        200,
        `<!doctype html><html><body><h1>Approve execution plan</h1>` +
          ...
          `<p>Possible: ${escapeHtml(review.impacts.possible.join(", ") || "none")}</p><form method="post">` +
    ...
    if (request.method === "POST") {
      ...
      this.respond(response, 200, "Execution plan approved.", "text/plain; charset=utf-8");
      return;
    }
